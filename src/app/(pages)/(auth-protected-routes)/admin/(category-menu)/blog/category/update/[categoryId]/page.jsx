...
export const generateMetadata = async ({ params }) => {
  ...
  const { categoryData, message } = await getPerticularCategory(
    userId,
    categoryId
  );
  ...
  return {
    title: message
      ? `Update Category ${process.env.NEXT_PUBLIC_META_APP_NAME}`
      : categoryData?.metaTitle ||
        `${categoryData.name} ${process.env.NEXT_PUBLIC_META_APP_NAME}`,
    description: message
      ? "Category details not found."
      : categoryData?.metaDescription ||
        `${categoryData.name} update form page.`,
  };
};
