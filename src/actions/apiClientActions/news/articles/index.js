...
export const getPerticularNewsArticle = async (userId, targetId) => {
...
export const updatePerticularNewsArticle = async (userId, targetId, data) => {
  try {
    ...
    if (response.data.success && response.status === 200) {
      ...
      await Promise.all([
        deleteCache(
          `${cacheKeyNames.NEWS_ARTICLE_DETAILS}-${targetId || "newsArticleId"}`
        ),
...
export const deletePerticularNewsArticle = async (userId, targetId) => {
