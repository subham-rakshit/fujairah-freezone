...
blogsCategorySchema.pre("save", async function (next) {
  try {
    const category = this;

    const existingCategory = await mongoose.models.Category.find({
    ...
    console.log("Error in category pre-save hook: ", error.message);
    next(error);
  }
});
